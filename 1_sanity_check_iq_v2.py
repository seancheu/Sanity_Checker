...
def create_unique_run_directory(base_out_dir: Union[str, Path], input_file: Path) -> Path:
    """
    Create a unique per-run output directory under the given base directory.

    Folder pattern: run_{input_stem}_{YYYYmmdd_%H%M%S}[_NN]
    If a folder with the timestamp already exists, appends an incrementing suffix.

    Args:
        base_out_dir: Base output directory (will be created if missing)
        input_file: Input file Path (used for naming)

    Returns:
        Path to the unique run directory
    """
    base = create_output_directory(base_out_dir)  # ensure base exists/writable
...
def main():
    ...
    try:
        ...
        # Create a unique per-run directory under the chosen base out_dir
        ...
        # Perform sanity check (note: output_dir is now the unique run_dir)
        ...
        # Generate output files inside the run_dir
        ...
        # Optional: save a lightweight run metadata file
        try:
            meta = {
                "run_dir": str(run_dir),
